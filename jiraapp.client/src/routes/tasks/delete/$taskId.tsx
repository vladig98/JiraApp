...
export const Route = createFileRoute('/tasks/delete/$taskId')({
...
function EditTaskPage() {
    ...
    async function handleSubmit(e: React.SubmitEvent) {
        ...
        try {
            ...
            setApiError("The server rejected the task deletion. No changes were made.")
            ...
            <div className="flex flex-col min-h-screen bg-slate-50/50">
                <header className="px-8 py-6 border-b border-slate-200 bg-white">
                    <nav className="text-[10px] text-slate-400 uppercase tracking-widest font-bold mb-1">
                        Projects / Boards / {board?.id} / Columns / {column?.id} / Tasks / Delete / {taskId}
                    </nav>
                    <h1 className="text-2xl font-bold text-slate-900">Delete Task from {column?.name}</h1>
                </header>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-3 bg-[#EBEBE4] border border-slate-300 rounded focus:ring-2 focus:ring-[#0052CC] outline-none text-lg shadow-sm"
                                />
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <textarea
                                    ...
                                    className="w-full px-4 py-3 bg-[#EBEBE4] border border-slate-300 rounded focus:ring-2 focus:ring-[#0052CC] outline-none resize-none shadow-sm"
                                />
                            </div>
                            ...
                            <div className="flex justify-end gap-3 pt-6 border-t border-slate-100">
                                ...
                                <button
                                    ...
                                    className="bg-[#880000] hover:bg-[#550000] disabled:bg-slate-300 text-white px-8 py-2.5 rounded font-bold shadow-md transition-all active:scale-95"
                                >
                                    {isSubmitting ? 'Optimizing...' : 'Delete Task'}
                                </button>
                            </div>
                        </form>
                    </div>
