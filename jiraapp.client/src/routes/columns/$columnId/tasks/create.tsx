...
export const Route = createFileRoute('/columns/$columnId/tasks/create')({
...
function CreateTaskPage() {
    ...
    const getBoardByColumnId = useBoardStore((state) => state.getBoardByColumnId)
    const upsertTask = useBoardStore((state) => state.upsertTask)
    const getColumn = useBoardStore((state) => state.getColumn)

    const board = getBoardByColumnId(columnId) as Board;
    const column = getColumn(columnId) as Column;
    ...
    async function handleSubmit(e: React.SubmitEvent) {
        ...
        try {
            const response = await fetch("/columns/" + columnId + "/tasks/", {
                method: 'POST',
            ...
            upsertTask(newTask, columnId)

            navigate({ to: '/boards/$boardId', params: { boardId: board.id } });
        } catch (error) {
            setApiError("The server rejected the new task. No changes were made.")
        } finally {
            ...
            <div className="flex flex-col min-h-screen bg-slate-50/50">
                <header className="px-8 py-6 border-b border-slate-200 bg-white">
                    <nav className="text-[10px] text-slate-400 uppercase tracking-widest font-bold mb-1">
                        Projects / Boards / {board?.id} / Columns / {column?.id} / Tasks / Create
                    </nav>
                    <h1 className="text-2xl font-bold text-slate-900">Add Task to {column?.name}</h1>
                </header>
                    ...
                    <div className="w-full max-w-2xl bg-white border border-slate-200 rounded-xl shadow-sm p-8">
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="flex justify-end gap-3 pt-6 border-t border-slate-100">
                                ...
                                >
                                    {isSubmitting ? 'Optimizing...' : 'Create Task'}
                                </button>
                            </div>
                        </form>
                    </div>
